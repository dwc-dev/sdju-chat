...
const PIC_UPLOAD_ADDRESS = "http://127.0.0.1:1002/upload";
const KEY_VERIFY_ADDRESS = "http://127.0.0.1:8080/VerifyServlet";
      ...
      verifyKey(msg.user_id, msg.key)
        .then((result) => {
          ...
          if (msg.option === "get_list") {
            let sql_query_usergroup = `SELECT * FROM \`Group\` WHERE \`group_id\` IN ( SELECT \`group_id\` FROM \`UserGroup\` WHERE \`user_id\` = ${msg.user_id} )`;
            ...
            let sql_query_userHeadIcon = `SELECT user_icon FROM \`User\` WHERE \`user_id\` = ${msg.user_id}`;
            ...
            let sql_1 = `SELECT message_id,sender_id,group_id,content,timestamp,user_icon,nickname FROM \`Message\`,\`User\` WHERE sender_id = user_id AND group_id = ${msg.chat_id}  ORDER BY \`timestamp\``;
            ...
            let sql_insert_groupMessage = `INSERT INTO \`Message\`( \`sender_id\`, \`group_id\`, \`content\`, \`timestamp\`) VALUES (${
              msg.sender_id
            ...
            con_mysql.query(sql_insert_groupMessage, (error, results) => {
              if (error) throw error;
              let sql_query_message = `SELECT message_id,sender_id,group_id,content,timestamp,user_icon,nickname FROM \`Message\`,\`User\` WHERE sender_id=user_id AND timestamp = ${timestamp}`;
              con_mysql.query(sql_query_message, (error, results) => {
                if (error) throw error;
                let newMsg = JSON.parse(JSON.stringify(results))[0];
                let sql_query_groupMenbers = `SELECT \`user_id\` FROM \`UserGroup\` WHERE \`group_id\`=${msg.group_id}`;
            ...
            let sql_add_group = `INSERT INTO \`UserGroup\` (\`user_id\`, \`group_id\`) VALUES ('${msg.user_id}', '${msg.group_id}');`;
            con_mysql.query(sql_add_group, (error, results) => {
              ...
              if (error) {
                ...
                let sql_find_groupName = `SELECT group_name FROM \`Group\` WHERE group_id='${msg.group_id}'`;
            ...
            axios
              ...
              .then((response) => {
                ...
                var iconUrl =
                  "https://chat.picbox.fun/pic/" + uploadData.name + ".webp";
                sql_1 = `INSERT INTO \`Group\` (\`group_id\`, \`group_name\`, \`group_create_time\`, \`group_icon\`) VALUES ('${group_id}', '${msg.group_name}', CURRENT_TIMESTAMP, '${iconUrl}')`;
                con_mysql.query(sql_1, (err, res) => {
                  ...
                  sql_2 = `INSERT INTO \`UserGroup\` (\`user_id\`, \`group_id\`) VALUES ('${msg.user_id}', '${group_id}')`;
            ...
            let sql_exit_group = `DELETE FROM \`UserGroup\` WHERE \`user_id\`=${msg.user_id} AND \`group_id\`=${msg.group_id}`;
            ...
            let sql_insert_groupMessage = `INSERT INTO \`Message\`( \`sender_id\`, \`group_id\`, \`content\`, \`timestamp\`) VALUES (${msg.user_id},${msg.group_id},'${content}',${timestamp})`;
