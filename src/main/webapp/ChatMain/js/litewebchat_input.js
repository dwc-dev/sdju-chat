    ...
    oLine.onmousedown = function (ev) {
        ...
        document.onmousemove = function (ev) {
            ...
            if (100 < upperHeight + diff && 100 < downHeight - diff) {
                //两个div的最小高度均为100px
                upperChild.style.height = "calc(100% - ".concat(
                    downHeight - diff,
                    "px)"
                );
    ...
    inputFile({
        ...
        sendFileFunc: function (file) {
            ...
            Swal.fire({
                ...
                didOpen: () => {
                    Swal.showLoading();
                    const progressItem = Swal.getPopup().querySelector("b");
                    timerInterval = setInterval(() => {
                        progressItem.innerHTML = `文件：${file.name}<br>进度：${progress.toFixed(2)}%，请耐心等待`;
                    }, 100);
                },
                willClose: () => {
            ...
            xhr.open('POST', FILE_UPLOAD_ADDRESS, true);
            xhr.responseType = 'json';

            xhr.upload.onprogress = function (event) {
                if (event.lengthComputable) {
                    let percentComplete = (event.loaded / event.total) * 100;
                    percentComplete = percentComplete === 100 ? 99.99 : percentComplete;
            ...
            xhr.onload = function () {
                Swal.close();
                if (xhr.status === 200 && xhr.response.result === "success") {
                    let msg_file = {
                        key: user_key,
                        user_id: user_id,
                        option: "upload_file",
                        group_id: current_chat.id,
                        fileName: xhr.response.fileName,
                        downloadUrl: xhr.response.downloadUrl,
                        fileSize: xhr.response.fileSize,
                        downloadWidget: xhr.response.downloadWidget,
                    };
                    socket.send(JSON.stringify(msg_file));
                    Toast.fire({
                        icon: "success",
                        title: "上传成功！"
                    });
