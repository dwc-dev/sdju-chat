...
function addMessage(chatboxClass, message, position) {
    var chatBox = document.querySelector(chatboxClass);
    var htmlStr = renderMessageHtml(message);
    chatBox.insertAdjacentHTML(position, htmlStr);
}

function addTip(chatboxClass, message, position) {
    var chatBox = document.querySelector(chatboxClass);
    var htmlStr = renderTipHtml(message.html, TipsType[message.messageType] || 'tips');
    chatBox.insertAdjacentHTML(position, htmlStr);
}

function renderMessageHtml(data) {
    return "<div class=\"c".concat(data.position, ` cmsg">\n        <img name="${data.user_id}" onclick="headClick(this)" style="cursor: pointer" class="headIcon `).concat(data.diamond ? '' : 'radius', "\" src=\"").concat(data.headIcon, "\" ondragstart=\"return false;\" oncontextmenu=\"return false;\"/>\n        <span class=\"name\">\n            ").concat(renderTitleHtml(data.htitle, TitleType[data.htitleType] || ''), "\n            <span>").concat(escapeHtml(data.name) || '&nbsp;', "</span>\n        </span>\n        <span class=\"content\">").concat(data.messageType === 'raw' ? data.html : escapeHtml(data.html), "</span>\n    </div>");
}

function headClick(item) {
    console.log(item.name);
    window.open("../ProfileServlet?user_id=" + item.name, "_blank");
}
